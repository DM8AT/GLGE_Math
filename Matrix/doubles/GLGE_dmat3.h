 ...
 * @file GLGE_dmat3.h
 * @author DM8AT
 * @brief define a 3x3 double matrix
 * @version 0.1
 * @date 2025-10-11
 * 
...
//header guard
#ifndef _GLGE_double_MAT_3x3_
#define _GLGE_double_MAT_3x3_

//include double 3D vectors
#include "../../Vector/doubles/GLGE_dvec3.h"

/**
 * @brief a 3x3 matrix of double elements
 */
typedef struct s_dmat3
{
    //store all possible lay outs for the data
    union {
        //use a 4 element (3*3) double array for data storage
        double m[9];
     ...
     */
    inline constexpr s_dmat3() noexcept
     : m{1,0,0,0,1,0,0,0,1}
    {}
     ...
     */
    inline constexpr s_dmat3(double m00, double m01, double m02,
     ...
     * @brief Construct a new 3*3 matrix
     * 
     * @param r0 a const reference to a 3D double vector for the top row
     * @param r1 a const reference to a 3D double vector for the middle row
     * @param r2 a const reference to a 3D double vector for the bottom row
     */
    inline constexpr s_dmat3(const dvec3& r0, const dvec3& r1, const dvec3& r2) noexcept
     : rows{r0,r1,r2} 
    {}
     ...
     */
    inline constexpr void operator=(const s_dmat3& mat) noexcept {
        rows[0] = mat.rows[0];
        rows[1] = mat.rows[1];
        rows[2] = mat.rows[2];
    }
     ...
     * @return s_dmat3 the sum of the two matrices
     */
    inline s_dmat3 operator+(s_dmat3 c) const noexcept {
        return s_dmat3(
            rows[0] + c.rows[0],
            rows[1] + c.rows[1],
            rows[2] + c.rows[2]
     ...
     * @return s_dmat3 the matrix + c
     */
    inline s_dmat3 operator+(double c) const noexcept {
        return s_dmat3(
            rows[0] + c,
            rows[1] + c,
            rows[2] + c
     ...
     * @param c the other matrix
     */
    inline constexpr void operator+=(s_dmat3 c) noexcept {
        rows[0].x += c.rows[0].x; rows[0].y += c.rows[0].y; rows[0].z += c.rows[0].z;
        rows[1].x += c.rows[1].x; rows[1].y += c.rows[1].y; rows[1].z += c.rows[0].z;
        rows[2].x += c.rows[2].x; rows[2].y += c.rows[2].y; rows[2].z += c.rows[0].z;
    }
     ...
     */
    inline constexpr void operator+=(double c) noexcept {
        rows[0].x += c; rows[0].y += c; rows[0].z += c;
        rows[1].x += c; rows[1].y += c; rows[1].z += c;
        rows[2].x += c; rows[2].y += c; rows[2].z += c;
    }
     ...
     * 
     * @param c the other matrix
     * @return s_dmat3 this matrix - the other matrix
     */
    inline s_dmat3 operator-(s_dmat3 c) const noexcept {
        return s_dmat3(
            rows[0] - c.rows[0],
     ...
     * 
     * @param c the constant value
     * @return s_dmat3 this matrix - the constant value
     */
    inline s_dmat3 operator-(double c) const noexcept {
        return s_dmat3(
            rows[0] - c,
    ...
    inline constexpr void operator-=(s_dmat3 c) noexcept {
        rows[0].x -= c.rows[0].x; rows[0].y -= c.rows[0].y; rows[0].z -= c.rows[0].z;
        rows[1].x -= c.rows[1].x; rows[1].y -= c.rows[1].y; rows[1].z -= c.rows[1].z;
        rows[2].x -= c.rows[2].x; rows[2].y -= c.rows[2].y; rows[2].z -= c.rows[2].z;
    }
    /**
     ...
     * 
     * @param c the other matrix
     * @return s_dmat3 the product of the two matrices
     */
    inline constexpr s_dmat3 operator*(s_dmat3 c) const noexcept {
        return s_dmat3(
            dvec3(
     ...
     * @return s_dmat3 the scaled matrix
     */
    inline s_dmat3 operator*(double s) const noexcept {
        return s_dmat3(
            rows[0] * s,
            rows[1] * s,
            rows[2] * s
     ...
     * 
     * @param v the vector
     * @return dvec3 the product of this matrix and the vector
     */
    inline constexpr dvec3 operator*(const dvec3& v) const noexcept {
        return dvec3(
    ...
    inline constexpr void operator*=(double s) noexcept {
        rows[0].x *= s; rows[0].y *= s; rows[0].z *= s;
        rows[1].x *= s; rows[1].y *= s; rows[1].z *= s;
        rows[2].x *= s; rows[2].y *= s; rows[2].z *= s;
    }
     ...
     * 
     * @return constexpr s_dmat3 the cofactor matrix
     */
    inline constexpr s_dmat3 cofactors() const noexcept {
        /**
         * A = {
         *  {a,b,c},
         *  {d,e,f},
     ...
     * 
     * @return constexpr s_dmat3 the transposed matrix
     */
    inline constexpr s_dmat3 transpose() const noexcept {
        return s_dmat3(
            dvec3(rows[0].x, rows[1].x, rows[2].x),
            dvec3(rows[0].y, rows[1].y, rows[2].y),
            dvec3(rows[0].z, rows[1].z, rows[2].z)
        );
    }

    /**
     * @brief compute the adjugate matrix of a 3*3 double matrix
     * 
     * @return constexpr s_dmat3 the adjugate matrix of this matrix
     */
    inline constexpr s_dmat3 adjugate() const noexcept {
     ...
     * 
     * @return s_dmat3 the inverse matrix
     */
    inline s_dmat3 inverse() const noexcept {
 ...
 */
void dmat3_addTo(dmat3* a, const dmat3* b);
 ...
 */
void dmat3_adddoubleTo(dmat3* a, double b);
 ...
 * @param a a constant pointer to the first matrix
 * @param b a constant pointer to the second matrix
 * @return dmat3 the difference of all elements
 */
dmat3 dmat3_subtract(const dmat3* a, const dmat3* b);
 ...
 * @return dmat3 the difference of the matrix and the scalar
 */
dmat3 dmat3_subtractdouble(const dmat3* a, double b);
 ...
 */
void dmat3_subtractFrom(dmat3* a, const dmat3* b);
 ...
 * @param b the double to subtract from the matrix
 */
void dmat3_subtractdoubleFrom(dmat3* a, double b);
 ...
 * @param a a constant pointer to the first matrix
 * @param b a constant pointer to the second matrix
 * @return dmat3 the product of both matrices
 */
dmat3 dmat3_multiply(const dmat3* a, const dmat3* b);
 ...
 * @return dmat3 the scaled matrix
 */
dmat3 dmat3_scale(const dmat3* a, double b);
 ...
 * @return dvec3 the product of the matrix and the vector
 */
dvec3 dmat3_apply(const dmat3* a, const dvec3* b);
 ...
 */
void dmat3_multiplyTo(dmat3* a, const dmat3* b);
 ...
 * @param b the double to scale the matrix with
 */
void dmat3_scaleTo(dmat3* a, double b);
 ...
 */
double dmat3_determinant(const dmat3* mat);
 ...
 * @return dmat3 the adjugate matrix
 */
dmat3 dmat3_adjugate(const dmat3* mat);

/**
 * @brief compute the transpose of a matrix
 * 
 * @param mat a constant pointer to a matrix to compute the transpose of
 * @return dmat3 the transposed matrix
 */
dmat3 dmat3_traspose(const dmat3* mat);

/**
 * @brief compute the inverse of a matrix
 * 
 * @param mat a constant pointer to a matrix to compute the inverse of
 * @return dmat3 the inverse matrix
 */
dmat3 dmat3_inverse(const dmat3* mat);
