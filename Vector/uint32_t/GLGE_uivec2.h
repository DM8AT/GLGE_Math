 ...
 */
inline float length(const uivec2& v) noexcept {return glge::sqrt((float)(v.x*v.x + v.y*v.y));}
 ...
 */
inline uivec2 normalize(const uivec2& v) noexcept {return v / (uint32_t)length(v);}
