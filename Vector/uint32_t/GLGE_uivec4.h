 ...
 */
inline float length(const uivec4& v) noexcept {return glge::sqrt((float)(v.x*v.x + v.y*v.y + v.z*v.z + v.w*v.w));}
 ...
 */
inline uivec4 normalize(const uivec4& v) noexcept {return v / (uint32_t)length(v);}
